...
Game* game_create() {
    ...
    menu_add_entry(game->menu, "Play",
                   (MENU_ENTRY_CALLBACK)game_go_play, game);
    menu_add_entry(game->menu, "Load level",
                   (MENU_ENTRY_CALLBACK)game_go_load, game);
    menu_add_entry(game->menu, "Create level",
                   (MENU_ENTRY_CALLBACK)game_go_editor, game);
    menu_add_entry(game->menu, "Exit",
                   (MENU_ENTRY_CALLBACK)game_go_exit, game);
...
void game_go_menu(Game* game) {
    ...
    if (SDL_FillRect(game->window->surface, NULL /*destrect*/,
                     SDL_MapRGB(game->window->surface->format, 255, 255, 255)) != 0) {
        fprintf(stderr, "%s\n", SDL_GetError());
    }
