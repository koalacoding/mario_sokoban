...
static Status load_sprites(MapView* map_view) {
  ...
  map_view->sprites[SQUARE_OBJECTIVE] = sprite_create("sprites/objective.png");
  map_view->sprites[SQUARE_MARIO] =
      sprite_create_faced("sprites/mario_up.gif", "sprites/mario_down.gif",
                          "sprites/mario_left.gif", "sprites/mario_right.gif");
...
static void destroy_sprites(MapView* map_view) {
  unsigned int i = 0;
  for (i = 0; i < map_view->sprite_count; i++) {
    if (map_view->sprites[i] == NULL) continue;
...
Status map_view_draw(MapView* map_view, SDL_Surface* surface,
  ...
  for (row = 0; row < map_view->map->row; row++) {
    ...
    for (column = 0; column < map_view->map->column; column++) {
      ...
      SDL_Rect rect;
      const Square* square =
          map_get_square(map_view->map, column, row, &status);
      if (square == NULL) {
        goto end;
      }
      rect.x = map_view->square_width * column + x_offset;
      rect.y = map_view->square_height * row + y_offset;
      ret = SDL_BlitSurface(get_sprite_surface(map_view, square), NULL, surface,
                            &rect);
      if (ret != 0) {
        status.message = SDL_GetError();
        goto end;
